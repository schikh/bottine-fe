...
<div class="container">
	...
	<div class="col p-4 d-flex flex-column position-static" *ngIf="model?.id">
			<h3 class="mb-0">{{model.title}}</h3>
			<div class="mb-1 text-muted">Nov 12</div>
			<div class="card-text mb-auto" [innerHTML]="model.text"></div>
			<div >{{model.id }}</div>

			<a [routerLink]="['/blogpost', 'edit', model.id]">Edit</a>
			<button class="btn btn-link" [routerLink]="['/blogpost', 'edit', model.id]" style="width: 50px;">Edit</button>
		</div>

	<div class="modal-footer" *ngIf="model?.id">
		<button type="button" class="btn btn-danger" [routerLink]="['/blogpost', 'edit', model.id]">Editer</button>
